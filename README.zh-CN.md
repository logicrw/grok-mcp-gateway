...
本地 Grok 模型 gateway，加上基于 Hermes/xAI OAuth 的 `x_retrieve` MCP，
供本地 AI Agent 客户端共用。

> `x_retrieve` 是基于 xAI `x_search` 的 generated best-effort 检索工具，
> 不是官方 X API timeline endpoint。
...
1. **模型接入：** 通过 OpenAI-compatible API 调用 Grok，不需要单独的
   xAI API key。
2. **工具接入：** 把 `x_retrieve` 暴露成常驻 HTTP MCP 工具，让非 Grok
   模型也能通过客户端工具层检索 X。
...
proxy 进程，同时获得 Grok 模型调用和 MCP X 检索能力。
...
> 常驻 HTTP MCP `x_retrieve` gateway，以及面向本地多 Agent 客户端的配置。
...
proxy 暴露的 MCP `x_retrieve` 工具，再由 proxy 使用你的本地 OAuth 会话
通过 xAI 完成检索。
...
- 一个本地 OpenAI-compatible Grok gateway，通过 Hermes 派生的 xAI OAuth
  调用 Grok 模型；
- 一个常驻 HTTP MCP server，暴露聚焦的 `x_retrieve` 工具；
...
**常驻 HTTP MCP X Retrieve**

在 `/mcp` 暴露共享的 `x_retrieve` 工具，供 Alma 和其他 MCP 客户端使用。
...
- `x_retrieve`：用于语义化 X 研究、结构化帖子检索、源头帖定位、社交反应追踪，以及单账号最新帖。

`x_search`、`x_posts`、`x_latest_posts` 已从 vNext 的公开 MCP 工具面移除。它们有用的能力会被 `x_retrieve` 内部 mode 保留，这样上层模型只需要选择一个工具。

### `x_retrieve`

| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `query` | string | 否 | 自然语言话题、传闻、信源线索、OCR 后的截图文字或研究问题。最多 `2000` 字符。`handles` + `sort=latest` 时可不填。 |
| `intent` | string | 否 | `auto`、`research`、`posts`、`source_discovery`、`reaction_tracking` 或 `verify_claim`。默认 `auto`。 |
| `handles` | string array | 否 | 可选作者账号，可带或不带 `@`。配合 `sort=latest` 可看账号最新帖。 |
| `excluded_handles` | string array | 否 | 未设置 `handles` 时可排除指定账号。 |
| `time_range` | string | 否 | 自然语言时间范围，例如 `上上周`、`最近30天`、`2026年4月` 或 `不限`。 |
| `from_date` | string | 否 | ISO8601 搜索起始日期。 |
| `to_date` | string | 否 | 包含当天的 ISO8601 搜索结束日期。date-only 值会原样传给 xAI。 |
| `count` | integer | 否 | 目标返回帖子数，默认 `10`，最大 `20`。 |
| `sort` | string | 否 | `latest` 或 `relevance`。有 `query` 的请求默认 `relevance`，仅按账号看最新帖时默认 `latest`。 |
| `lookback_days` | integer | 否 | `handles` + `sort=latest` 且未设置日期时的回看窗口，默认 `30` 天。 |
...
raw expansion 会在 quality gate 判断需要更多候选时自动触发，例如源头帖定位、
...
`GROK_PROXY_RETRIEVE_MODEL`、`GROK_PROXY_MCP_MODEL` 或 raw expansion 模型到更新
或账号专属模型前，先用 `/v1/models` 和 live MCP smoke test 验证。
...
| `/mcp` | `POST` | HTTP JSON-RPC MCP endpoint，默认暴露 `x_retrieve`。 |
