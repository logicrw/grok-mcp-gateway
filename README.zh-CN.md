...
3. **旗舰智能通道（Smart Lane: `grok-4.6` 旗舰模型）**：
   ...
   - 自适应挂载合规的推理档位（`low` / `medium` / `high` / `xhigh`）与多轮 Agentic `x_search` 工具链。日常 Smart 检索默认 `medium`，事实核查 `verify_claim` 默认 `high`。在 grok-4.6 上显式传入 `xhigh` 会原样转发给 Responses API。
...
3. **客户端 60 秒红线默认收敛**：日常查询必须收敛在 35 秒内返回，天然兼容第三方 60s 客户端；深度长推演明确声明长耗时；
