...
## 铁律三：客户端 60 秒红线免疫（Client Ceiling Invariance）
- **严禁依赖客户端放宽超时**：行业内主流 MCP 客户端（如 Claude Desktop 桌面版、部分 IDE 插件）内置了不可修改的 60 秒前端硬截断。
- **服务端主动收敛**：
  - 日常 90% 以上的无特殊修饰查询，必须在 **35 秒以内** 完成并交付（Fast Lane 3~6s，Smart low 29~35s）；
  - 保证无论接入哪一个 Agent 客户端，在无需任何特殊配置的前提下，全链路天然免疫 60s 超时报错。
