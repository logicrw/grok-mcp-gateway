...
| --- | --- | --- | --- |
| Explicit status URL or ID | Grok 4.5, low reasoning | Concurrent public oEmbed, then one batched Grok 4.5 exact fallback | Composer is never used |
| Latest by handle | Grok 4.5, low reasoning | Quality and target validation | Composer only when stable output is empty |
| Structured posts | Grok 4.5, low reasoning | Schema normalization and quality gate | Composer only when the quality gate fails |
| Research, source, reactions | Grok 4.5, medium reasoning | Schema normalization and quality gate | Composer only when the quality gate fails |
| Claim verification | Grok 4.5, high reasoning | Schema normalization and target validation | Composer only when allowed by policy |
...
One `x_retrieve` request has a 120-second total deadline and a 60-second generative-stage ceiling. The deadline wraps the complete stage, including semaphore wait, OAuth refresh, retry, and response parsing. Later stages receive only the remaining budget.
...
`/metrics` records final status, stage/model/status/reasoning effort, timeout boundary, bounded error kind, reasoning tokens, and server-side X Search calls. No response body, prompt, OAuth value, or user content is used as a metric label.
